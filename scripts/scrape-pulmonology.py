...
# Keep order in sync with src/lib/procedure-map.ts PULMONOLOGY_RULES.
PULMONOLOGY_RULES: list[tuple[re.Pattern, str]] = [
    (re.compile(r"\bebus\b|endobronchial ultrasound", re.I), "EBUS (Endobronchial Ultrasound)"),
    (re.compile(r"rigid bronchoscop", re.I), "Rigid Bronchoscopy"),
    (re.compile(r"airway stent|tracheal stent|bronchial stent", re.I), "Airway Stenting"),
    (re.compile(r"thoracoscop", re.I), "Medical Thoracoscopy"),
    (re.compile(r"pleuroscop", re.I), "Medical Pleuroscopy"),
    (re.compile(r"cryo[\s-]*lung|cryobiops|cryo[\s-]*biops", re.I), "Cryo-Lung Biopsy"),
    (re.compile(r"transbronchial lung|\btblb\b", re.I), "Transbronchial Lung Biopsy"),
    (re.compile(r"endobronchial biops", re.I), "Endobronchial Biopsy"),
    (re.compile(r"\btbna\b|transbronchial needle", re.I), "TBNA (Transbronchial Needle Aspiration)"),
    (re.compile(r"debulk|tumou?r debulk", re.I), "Bronchoscopic Tumor Debulking"),
    (re.compile(r"foreign body.{0,24}bronch|inhaled foreign|airway foreign", re.I), "Foreign Body Removal by Bronchoscopy"),
    (re.compile(r"pleural biops", re.I), "Pleural Biopsy"),
    (re.compile(r"chest tube|intercostal drain|\bicd\b", re.I), "Chest Tube / Intercostal Drainage"),
    (re.compile(r"lung transplant", re.I), "Lung Transplantation"),
    (re.compile(r"bronchoscop", re.I), "Bronchoscopy"),
]
...
def map_procedures(texts: list[str]) -> list[str]:
    ...
    for test, name in PULMONOLOGY_RULES:
        if test.search(blob) and name not in found:
            found.append(name)
    if re.search(r"interventional pulmonol", blob, re.I):
        for name in (
            "Bronchoscopy",
            "EBUS (Endobronchial Ultrasound)",
...
def pronoun_from(text: str, name: str) -> str:
    ...
    if any(t in FEMALE_FIRST for t in tokens):
        return "she"
    return "he"
...
def write_bio(d: dict) -> str:
    name = d["name"]
    title = d["designation"] or "Consultant, Pulmonology"
    ...
    years_bit = (
        f"{cap} has practised pulmonology in India for {years.lower()}."
        if years
        else f"{cap} practises pulmonology in India."
    )
    edu_bit = f" Training includes {edu}." if edu else ""
    return (
        f"{name} is {title} at {hospital} in {city}, India. {years_bit} "
        f"Airway and pleural lists are tagged for later pSEO under country, city, specialty and procedure — "
        f"{p1}, {p2} and {p3} when CT, bronchoscopy notes and gas exchange already make the indication honest. "
        f"Flexible versus rigid access, EBUS versus conventional TBNA, thoracoscopy versus a closed pleural biopsy, "
        f"and whether lung transplant belongs on the same campus are written after records review — not from a day-care brochure.{edu_bit} "
        f"International patients meet {obj} on camera first; travel to {city} is offered only if this respiratory floor is the right list."
    )
...
def main() -> None:
    ...
    catalog["doctors"] = [d for d in catalog["doctors"] if d.get("specialty") != "Pulmonology"]
