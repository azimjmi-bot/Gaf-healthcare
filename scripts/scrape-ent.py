#!/usr/bin/env python3
"""Fetch structured fields from Ginger's India ENT listing. Bios are written here, not copied."""
...
LISTING = Path("/tmp/ginger-ent.html")
OUT_DOCTORS = Path("/tmp/ent-doctors.json")
OUT_HOSPITALS = Path("/tmp/ent-hospitals.json")
CATALOG = Path("/workspace/src/data/ginger-catalog.json")
UA = {"User-Agent": "Mozilla/5.0 (compatible; GAF HealthcareCatalog/1.0)"}

ENT_RULES: list[tuple[re.Pattern, str]] = [
    (re.compile(r"tors|transoral robotic", re.I), "Transoral Robotic Surgery (TORS)"),
    (re.compile(r"cochlear", re.I), "Cochlear Implantation"),
    (re.compile(r"baha|bone[\s-]*anchored", re.I), "BAHA Implantation (Bone Anchored Hearing Aid)"),
    (re.compile(r"balloon sinus", re.I), "Balloon Sinuplasty"),
    (re.compile(r"\bfess\b|endoscopic sinus", re.I), "FESS (Functional Endoscopic Sinus Surgery)"),
    (re.compile(r"staped", re.I), "Stapedectomy / Stapedotomy"),
    (re.compile(r"skull[\s-]*base", re.I), "Skull Base Surgery"),
    (re.compile(r"head[\s&]*neck cancer|head and neck cancer", re.I), "Head & Neck Cancer Surgery"),
    (re.compile(r"microlaryngeal|microlaryng", re.I), "Microlaryngeal Surgery"),
    (re.compile(r"vocal cord|voice", re.I), "Vocal Cord Surgery"),
    (re.compile(r"sleep apnea|osa|uppp", re.I), "Sleep Apnea Surgery"),
    (re.compile(r"mastoid", re.I), "Mastoidectomy"),
    (re.compile(r"tympanoplasty|myringoplasty", re.I), "Tympanoplasty"),
    (re.compile(r"adenoid", re.I), "Adenoidectomy"),
    (re.compile(r"tonsil", re.I), "Tonsillectomy"),
    (re.compile(r"septoplasty|deviated septum", re.I), "Septoplasty"),
    (re.compile(r"rhinoplasty", re.I), "Rhinoplasty"),
    (re.compile(r"thyroid", re.I), "Thyroid Surgery"),
]
...
SOURCE = "https://ginger.healthcare/destinations/india/ent/"
...
def write_bio(d: dict) -> str:
    name = d["name"]
    title = d["designation"] or "Consultant, ENT"
    ...
    p1 = procs[0] if procs else "FESS (Functional Endoscopic Sinus Surgery)"
    p2 = procs[1] if len(procs) > 1 else "Septoplasty"
    p3 = procs[2] if len(procs) > 2 else "Tonsillectomy"
    edu = d["education"][0] if d["education"] else ""
    pron = d["_pronoun"]
    obj = "her" if pron == "she" else "him"
    cap = "She" if pron == "she" else "He"
    years_bit = (
        f"{cap} has practised ENT surgery in India for {years.lower()}."
        if years
        else f"{cap} practises ENT surgery in India."
    )
    edu_bit = f" Training includes {edu}." if edu else ""
    return (
        f"{name} is {title} at {hospital} in {city}, India. {years_bit} "
        f"The ENT list covers {p1}, {p2} and {p3} when imaging and hearing tests make the indication honest. "
        f"Approach, implant and reconstruction are written after records review — not from a brochure.{edu_bit} "
...
def parse_hospital(html: str, slug: str, href: str, city: str, name_hint: str) -> dict:
    ...
    bio = (
        f"{name} is an ENT campus in {city}, India on GAF Healthcare's list. "
        f"{'It opened in ' + year + '. ' if year else ''}"
        f"{(beds + ' beds. ') if beds else ''}"
        f"{' · '.join(acc) + ' accreditation is current. ' if acc else ''}"
        f"Named ENT surgeons write cochlear implants, FESS and septoplasty here; "
        f"oncology, cardiac, bariatric and cosmetic lists sit on the same map when the case needs them. "
