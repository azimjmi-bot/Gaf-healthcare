...
# Keep order in sync with src/lib/procedure-map.ts SPINE_RULES.
SPINE_RULES: list[tuple[re.Pattern, str]] = [
    (re.compile(r"\bplif\b|posterior lumbar interbody", re.I), "PLIF (Posterior Lumbar Interbody Fusion)"),
    (re.compile(r"\btlif\b|transforaminal lumbar", re.I), "TLIF (Transforaminal Lumbar Interbody Fusion)"),
    (re.compile(r"\balif\b|anterior lumbar interbody", re.I), "ALIF (Anterior Lumbar Interbody Fusion)"),
    (re.compile(r"\bacdf\b|anterior cervical discectomy", re.I), "ACDF (Anterior Cervical Discectomy and Fusion)"),
    (re.compile(r"microdiscect|micro[\s-]*discect", re.I), "Microdiscectomy"),
    (re.compile(r"discect|diskect", re.I), "Discectomy"),
    (re.compile(r"laminect", re.I), "Laminectomy"),
    (re.compile(r"kyphoplast", re.I), "Kyphoplasty"),
    (re.compile(r"vertebroplast", re.I), "Vertebroplasty"),
    (re.compile(r"disc replacement|arthroplasty|artificial disc|\badr\b", re.I), "Disc Replacement"),
    (re.compile(r"scoliosis", re.I), "Scoliosis Correction"),
    (re.compile(r"deformity", re.I), "Spinal Deformity Correction"),
    (re.compile(r"revision spine|revision fusion|failed back", re.I), "Revision Spine Surgery"),
    (re.compile(r"spinal tumor|spine tumor|intradural|intramedullary|metastatic spine", re.I), "Spinal Tumor Surgery"),
    (re.compile(r"decompress", re.I), "Spinal Decompression"),
    (re.compile(r"spinal fusion|lumbar fusion|cervical fusion|instrumented fusion", re.I), "Spinal Fusion"),
]
...
def write_bio(d: dict) -> str:
    name = d["name"]
    title = d["designation"] or "Consultant, Spine Surgery"
    ...
    years_bit = (
        f"{cap} has practised spine surgery in India for {years.lower()}."
        if years
        else f"{cap} practises spine surgery in India."
    )
    ...
    return (
        ...
        f"{p1}, {p2} and {p3} when MRI and neurology already make the indication honest. "
        f"Fusion versus motion-preservation, ACDF versus disc replacement, and deformity versus "
        f"revision work-up are written after records review — not from a brochure segment count.{edu_bit} "
        f"International patients meet {obj} on camera first; travel to {city} is offered only if this spine floor is the right list."
    )
...
def main() -> None:
    ...
    for d in sorted(enriched, key=lambda x: listing_order.get(x["slug"], 10_000)):
        ...
        if not d.get("specializations"):
            d["specializations"] = ["Spine Surgery"]
        doctors.append(d)
