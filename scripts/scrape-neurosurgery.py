#!/usr/bin/env python3
"""Fetch structured fields from Ginger's India neurosurgery listing.

Bios are original GAF Healthcare copy (brain tumour, aneurysm, DBS, radiosurgery).
Do not paste Ginger marketing.
Source: https://ginger.healthcare/destinations/india/neurosurgery/
Specialty on GAF Healthcare is Neurosurgery (slug neurosurgery).
"""
...
LISTING = Path("/tmp/ginger-neurosurgery.html")
OUT_DOCTORS = Path("/tmp/neurosurgery-doctors.json")
...
NEUROSURGERY_RULES: list[tuple[re.Pattern, str]] = [
    (re.compile(r"glioma", re.I), "Glioma Surgery"),
    (re.compile(r"meningioma", re.I), "Meningioma Surgery"),
    (re.compile(r"pituitary", re.I), "Pituitary Tumor Surgery"),
    (re.compile(r"spinal tumor|spine tumor|intradural|intramedullary", re.I), "Spinal Tumor Surgery"),
    (re.compile(r"brain tumor|brain tumour|craniotom", re.I), "Brain Tumor Surgery"),
    (re.compile(r"endoscopic skull|endonasal|transsphenoidal", re.I), "Endoscopic Skull Base Surgery"),
    (re.compile(r"endoscopic brain|neuroendoscop", re.I), "Endoscopic Brain Surgery"),
    (re.compile(r"skull base", re.I), "Skull Base Surgery"),
    (re.compile(r"stereotactic.{0,12}biopsy|brain biopsy", re.I), "Stereotactic Brain Biopsy"),
    (re.compile(r"aneurysm clip", re.I), "Aneurysm Clipping"),
    (re.compile(r"aneurysm coil|coiling", re.I), "Aneurysm Coiling"),
    (re.compile(r"avm embol", re.I), "AVM Embolization"),
    (re.compile(r"\bavm\b|arteriovenous malformation", re.I), "AVM Surgery"),
    (re.compile(r"thrombectomy|mechanical thromb", re.I), "Stroke Thrombectomy"),
    (re.compile(r"cerebral bypass|sta[\s-]*mca|moyamoya", re.I), "Cerebral Bypass"),
    (re.compile(r"\bdbs\b|deep brain stim", re.I), "Deep Brain Stimulation"),
    (re.compile(r"epilepsy", re.I), "Epilepsy Surgery"),
    (re.compile(r"gamma\s*knife", re.I), "Gamma Knife"),
    (re.compile(r"cyber\s*knife", re.I), "CyberKnife"),
    (re.compile(r"\bsrs\b|stereotactic radio", re.I), "Stereotactic Radiosurgery (SRS)"),
    (re.compile(r"stereotactic", re.I), "Stereotactic Brain Surgery"),
    (re.compile(r"\betv\b|third ventriculostom", re.I), "Endoscopic Third Ventriculostomy (ETV)"),
    (re.compile(r"hydrocephalus|vp shunt|ventriculoperitoneal", re.I), "Hydrocephalus Surgery"),
    (re.compile(r"chiari", re.I), "Chiari Surgery"),
    (re.compile(r"craniosynostos", re.I), "Craniosynostosis Surgery"),
]

FALLBACK = ["Brain Tumor Surgery", "Aneurysm Clipping", "Deep Brain Stimulation"]
...
def map_procedures(texts: list[str]) -> list[str]:
    ...
    for test, name in NEUROSURGERY_RULES:
...
def write_bio(d: dict) -> str:
    name = d["name"]
    title = d["designation"] or "Consultant, Neurosurgery"
    ...
    years_bit = (
        f"{cap} has practised neurosurgery in India for {years.lower()}."
        if years
        else f"{cap} practises neurosurgery in India."
    )
    edu_bit = f" Training includes {edu}." if edu else ""
    featured_bit = (
        f" GAF Healthcare lists {obj} among featured India neurosurgeons for brain-tumour, aneurysm and radiosurgery lists."
        if d.get("featured")
        else ""
    )
    return (
        f"{name} is {title} at {hospital} in {city}, India. {years_bit}{featured_bit} "
        f"Brain tumour, skull-base, aneurysm, DBS, paediatric and radiosurgery lists are tagged for later pSEO under country, city, specialty and procedure — "
        f"{p1}, {p2} and {p3} when MRI, angiography or EEG already make the indication honest. "
        f"Craniotomy versus endoscopic corridor, clipping versus coiling, and open resection versus Gamma Knife, CyberKnife or SRS "
        f"are written after records review — not from a brochure robot or frame count.{edu_bit} "
        f"International patients meet {obj} on camera first; travel to {city} is offered only if this neurosurgery floor is the right list."
    )
