...
# Keep order in sync with src/lib/procedure-map.ts PEDIATRIC_ORTHOPAEDIC_RULES.
PEDIATRIC_ORTHOPAEDIC_RULES: list[tuple[re.Pattern, str]] = [
    (re.compile(r"\bscfe\b|slipped capital femoral", re.I), "SCFE Hip Surgery (Slipped Capital Femoral Epiphysis)"),
    (re.compile(r"\bddh\b|developmental dysplasia|hip dysplasia", re.I), "Developmental Dysplasia of Hip Surgery"),
    (re.compile(r"clubfoot|talipes|\bctev\b|ponseti", re.I), "Clubfoot Correction Surgery"),
    (re.compile(r"pediatric scoliosis|paediatric scoliosis|child.{0,12}scoliosis", re.I), "Pediatric Scoliosis Surgery"),
    (re.compile(r"\bscoliosis\b|growing rod", re.I), "Pediatric Scoliosis Surgery"),
    (
        re.compile(r"pediatric spinal deformity|paediatric spinal deformity|congenital scoliosis|neuromuscular scoliosis", re.I),
        "Pediatric Spinal Deformity Correction",
    ),
    (re.compile(r"cerebral palsy|semls\b|single-event multilevel", re.I), "Cerebral Palsy Orthopedic Surgery"),
    (re.compile(r"hip preservation|periacetabular|impingement", re.I), "Hip Preservation Surgery"),
    (re.compile(r"pediatric hip reconstr|paediatric hip reconstr|pelvic osteotomy|perthes", re.I), "Pediatric Hip Reconstruction"),
    (re.compile(r"limb lengthen|ilizarov.{0,12}length", re.I), "Limb Lengthening Surgery"),
    (re.compile(r"limb reconstr", re.I), "Limb Reconstruction Surgery"),
    (re.compile(r"pediatric foot|paediatric foot|pediatric ankle|paediatric ankle", re.I), "Pediatric Foot & Ankle Surgery"),
    (re.compile(r"tendon repair|tendon transfer", re.I), "Tendon Repair Surgery"),
    (re.compile(r"fracture fixation|k[\s-]*wire|elastic nailing|\btens\b", re.I), "Pediatric Fracture Fixation"),
    (re.compile(r"pediatric fracture|paediatric fracture|child.{0,12}fracture", re.I), "Pediatric Fracture Surgery"),
    (re.compile(r"pediatric deformity|paediatric deformity|limb deformity|\bdeformity\b", re.I), "Pediatric Deformity Correction"),
]
...
def map_procedures(texts: list[str]) -> list[str]:
    ...
    for test, name in PEDIATRIC_ORTHOPAEDIC_RULES:
...
def write_bio(d: dict) -> str:
    name = d["name"]
    title = d["designation"] or "Consultant, Pediatric Orthopaedic"
    ...
    years_bit = (
        f"{cap} has practised paediatric orthopaedics in India for {years.lower()}."
        if years
        else f"{cap} practises paediatric orthopaedics in India."
    )
    edu_bit = f" Training includes {edu}." if edu else ""
    return (
        f"{name} is {title} at {hospital} in {city}, India. {years_bit} "
        f"Children’s limb and spine lists are tagged for later pSEO under country, city, specialty and procedure — "
        f"{p1}, {p2} and {p3} when growth remaining, films and gait already make the indication honest. "
        f"Clubfoot versus DDH, SCFE versus hip reconstruction, lengthening versus reconstruction, and paediatric scoliosis versus "
        f"adult spine lists are written after records review — not from a brochure implant count.{edu_bit} "
        f"International families meet {obj} on camera first; travel to {city} is offered only if this children’s orthopaedic floor is the right list."
    )
