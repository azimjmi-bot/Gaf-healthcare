#!/usr/bin/env python3
"""Fetch structured fields from Ginger's India hematology listing. Bios are written here, not copied."""
...
HEMATOLOGY_RULES: list[tuple[re.Pattern, str]] = [
    ...
    (re.compile(r"bone marrow", re.I), "Bone Marrow Transplantation"),
    (re.compile(r"stem cell|transplant", re.I), "Stem Cell Transplantation"),
]
...
def map_procedures(texts: list[str]) -> list[str]:
    ...
    for test, name in HEMATOLOGY_RULES:
        if test.search(blob) and name not in found:
            found.append(name)
    return found or ["Bone Marrow Transplantation", "Stem Cell Transplantation", "Bone Marrow Biopsy"]
...
def pronoun_from(text: str, name: str) -> str:
    ...
    if re.search(r"\bhe\b|\bhis\b|\bhim\b", t):
        return "he"
    n = name.lower()
    if any(x in n for x in ("divya", "tina", "manisha", "mehak", "meet kumar")):
        if "meet kumar" in n:
            return "he"
        return "she"
    return "he"
...
def write_bio(d: dict) -> str:
    name = d["name"]
    title = d["designation"] or "Consultant, Hematology"
    ...
    p1 = procs[0] if procs else "bone marrow transplantation"
    p2 = procs[1] if len(procs) > 1 else "stem cell transplantation"
    ...
    years_bit = (
        f"{cap} has practised haematology in India for {years.lower()}."
        if years
        else f"{cap} practises haematology in India."
    )
    edu_bit = f" Training includes {edu}." if edu else ""
    return (
        f"{name} is {title} at {hospital} in {city}, India. {years_bit} "
        f"The marrow list covers {p1}, {p2} and {p3} when the indication is honest. "
        f"Donor, conditioning and graft source are written after records review — not from a brochure.{edu_bit} "
        f"International patients meet {obj} on camera first; travel to {city} is offered only if this unit is the right floor."
    )
...
def parse_hospital(html: str, slug: str, href: str, city: str, name_hint: str) -> dict:
    ...
    bio = (
        ...
        f"Named haematologists write transplant, CAR-T and marrow-diagnostic protocols here; "
        f"medical, radiation and surgical oncology sit on the same map when the case needs them. "
...
def main() -> None:
    html = LISTING.read_text(errors="replace") if LISTING.exists() else fetch(
        "https://ginger.healthcare/destinations/india/hematology/"
    )
