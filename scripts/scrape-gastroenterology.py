#!/usr/bin/env python3
"""Fetch structured fields from Ginger's India gastroenterology listing. Bios are written here, not copied."""
...
LISTING = Path("/tmp/ginger-gastro.html")
...
GI_RULES: list[tuple[re.Pattern, str]] = [
    ...
    (re.compile(r"\bptbd\b|percutaneous transhepatic", re.I), "PTBD (Percutaneous Transhepatic Biliary Drainage)"),
    (re.compile(r"g[\s-]*poem|gastroparesis.{0,20}myotomy|pyloromyotomy", re.I), "G-POEM"),
    (re.compile(r"z[\s-]*poem|zenker", re.I), "Z-POEM"),
    (re.compile(r"\bpoem\b|peroral endoscopic myotomy|achalasia.{0,12}myotomy", re.I), "Peroral Endoscopic Myotomy (POEM)"),
    (re.compile(r"\bster\b|submucosal tunneling endoscopic", re.I), "STER (Submucosal Tunneling Endoscopic Resection)"),
    (re.compile(r"\besd\b|submucosal dissection", re.I), "Endoscopic Submucosal Dissection (ESD)"),
    (re.compile(r"\bemr\b|mucosal resection", re.I), "Endoscopic Mucosal Resection (EMR)"),
    (re.compile(r"cholangioscop|spyglass", re.I), "Cholangioscopy"),
    (re.compile(r"biliary stent|metal stent|plastic stent", re.I), "Biliary Stenting"),
    (re.compile(r"bile duct stone|choledocholith|cbd stone", re.I), "Bile Duct Stone Removal"),
    (re.compile(r"\bercp\b|endoscopic retrograde", re.I), "ERCP"),
    (re.compile(r"\beus\b|endoscopic ultrasound", re.I), "Endoscopic Ultrasound (EUS)"),
    (re.compile(r"capsule", re.I), "Capsule Endoscopy"),
    (re.compile(r"enteroscop", re.I), "Enteroscopy"),
    (re.compile(r"variceal|banding|\bevl\b", re.I), "Variceal Band Ligation"),
    (re.compile(r"hemostasis|haemostasis|gi bleed|ulcer bleed", re.I), "Endoscopic Hemostasis"),
    (re.compile(r"foreign body|ingested", re.I), "Foreign Body Removal"),
    (re.compile(r"liver biopsy", re.I), "Liver Biopsy"),
    (re.compile(r"esophageal manometr|oesophageal manometr", re.I), "Esophageal Manometry"),
    (re.compile(r"anorectal manometr", re.I), "Anorectal Manometry"),
    (re.compile(r"bariatric.{0,20}endoscop|metabolic endoscop|endoscopic sleeve|gastric balloon", re.I), "Bariatric / Metabolic Endoscopy"),
    (re.compile(r"colonoscop", re.I), "Colonoscopy"),
    (re.compile(r"gastroscop|upper gi|\bogd\b|\begd\b|oesophagogastro", re.I), "Upper GI Endoscopy (Gastroscopy)"),
]
...
SOURCE = "https://ginger.healthcare/destinations/india/gastroenterology/"
...
def write_bio(d: dict) -> str:
    name = d["name"]
    title = d["designation"] or "Consultant, Gastroenterology"
    ...
    p1 = procs[0] if procs else "Upper GI Endoscopy (Gastroscopy)"
    p2 = procs[1] if len(procs) > 1 else "Colonoscopy"
    p3 = procs[2] if len(procs) > 2 else "ERCP"
    edu = d["education"][0] if d["education"] else ""
    pron = d["_pronoun"]
    obj = "her" if pron == "she" else "him"
    cap = "She" if pron == "she" else "He"
    years_bit = (
        f"{cap} has practised gastroenterology in India for {years.lower()}."
        if years
        else f"{cap} practises gastroenterology in India."
    )
    edu_bit = f" Training includes {edu}." if edu else ""
    return (
        f"{name} is {title} at {hospital} in {city}, India. {years_bit} "
        f"The endoscopy list covers {p1}, {p2} and {p3} when imaging and labs make the indication honest. "
        f"Stent, histology and whether ERCP follows are written after records review — not from a brochure.{edu_bit} "
        f"International patients meet {obj} on camera first; travel to {city} is offered only if this suite is the right floor."
    )
...
def parse_hospital(html: str, slug: str, href: str, city: str, name_hint: str) -> dict:
    ...
    bio = (
        ...
        f"Named gastroenterologists write ERCP, colonoscopy and EUS here; "
        f"oncology, cardiac, bariatric, cosmetic and ENT lists sit on the same map when the case needs them. "
...
def main() -> None:
    ...
    for d in sorted(enriched, key=lambda x: x["name"]):
        ...
        if not d.get("specializations"):
            d["specializations"] = ["Gastroenterology"]
        doctors.append(d)
