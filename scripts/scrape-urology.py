#!/usr/bin/env python3
"""Fetch structured fields from Ginger's India urology listing.

Bios are original GAF Healthcare copy (stone lists, prostate, kidney transplant,
reconstructive and paediatric urology). Do not paste Ginger marketing.
Source: https://ginger.healthcare/destinations/india/urology/
"""
...
LISTING = Path("/tmp/ginger-urology.html")
OUT_DOCTORS = Path("/tmp/urology-doctors.json")
CATALOG = Path("/workspace/src/data/ginger-catalog.json")
UA = {"User-Agent": "Mozilla/5.0 (compatible; GAF HealthcareCatalog/1.0)"}

SOURCE = "https://ginger.healthcare/destinations/india/urology/"

# Keep order in sync with src/lib/procedure-map.ts UROLOGY_RULES.
UROLOGY_RULES: list[tuple[re.Pattern, str]] = [
    ...
    (re.compile(r"\bpcnl\b|percutaneous nephrolith", re.I), "PCNL (Percutaneous Nephrolithotomy)"),
    (re.compile(r"\brirs\b|retrograde intrarenal", re.I), "RIRS (Retrograde Intrarenal Surgery)"),
    (re.compile(r"\beswl\b|shock[\s-]*wave lithotrips", re.I), "ESWL (Extracorporeal Shock Wave Lithotripsy)"),
    (re.compile(r"ureteroscop", re.I), "Ureteroscopy"),
    (re.compile(r"pyeloplast", re.I), "Pyeloplasty"),
    (re.compile(r"radical nephr", re.I), "Radical Nephrectomy"),
    (re.compile(r"partial nephr", re.I), "Partial Nephrectomy"),
    (re.compile(r"\bholep\b|holmium.{0,20}enucleat", re.I), "HoLEP (Holmium Laser Enucleation)"),
    (re.compile(r"green[\s-]*light|photoselective vapori", re.I), "GreenLight Laser Surgery"),
    (re.compile(r"\bturp\b|transurethral resection of the prostate", re.I), "TURP (Transurethral Resection of the Prostate)"),
    (re.compile(r"prostatect", re.I), "Radical Prostatectomy"),
    (re.compile(r"\bturbt\b|transurethral resection of bladder", re.I), "TURBT (Transurethral Resection of Bladder Tumor)"),
    (re.compile(r"cystect", re.I), "Radical Cystectomy"),
    (re.compile(r"bladder reconstr|neobladder|augmentation cystoplast", re.I), "Bladder Reconstruction"),
    (re.compile(r"urinary diversion|ileal conduit", re.I), "Urinary Diversion"),
    (re.compile(r"urethroplast", re.I), "Urethroplasty"),
    (re.compile(r"\bviu\b|internal urethrotom", re.I), "VIU (Visual Internal Urethrotomy)"),
    (re.compile(r"urinary tract reconstr|ureteric reimplant|ureteral reconstr", re.I), "Urinary Tract Reconstruction"),
    (re.compile(r"hypospadias", re.I), "Hypospadias Repair"),
    (re.compile(r"pediatric urolog|paediatric urolog", re.I), "Pediatric Urological Surgery"),
    (re.compile(r"penile implant|penile prosthes", re.I), "Penile Implant"),
    (re.compile(r"varicocele", re.I), "Varicocele Surgery"),
]
...
def map_procedures(texts: list[str]) -> list[str]:
    ...
    for test, name in UROLOGY_RULES:
...
def write_bio(d: dict) -> str:
    name = d["name"]
    title = d["designation"] or "Consultant, Urology"
    ...
    years_bit = (
        f"{cap} has practised urology in India for {years.lower()}."
        if years
        else f"{cap} practises urology in India."
    )
    edu_bit = f" Training includes {edu}." if edu else ""
    return (
        f"{name} is {title} at {hospital} in {city}, India. {years_bit} "
        f"Theatre and laser lists are tagged for later pSEO under country, city, specialty and procedure — "
        f"{p1}, {p2} and {p3} when CT, PSA or GFR already make the indication honest. "
        f"Stone clearance, prostate resection or enucleation, and living- versus deceased-donor kidney work-up "
        f"are written after records review — not from a brochure package.{edu_bit} "
        f"International patients meet {obj} on camera first; travel to {city} is offered only if this urology floor is the right list."
    )
