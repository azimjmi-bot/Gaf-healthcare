#!/usr/bin/env python3
"""Fetch structured fields from Ginger's India nephrology listing.

Bios are original GAF Healthcare copy (dialysis, transplant, biopsy, access).
Do not paste Ginger marketing.
Source: https://ginger.healthcare/destinations/india/nephrology/
Specialty on GAF Healthcare is Nephrology (slug nephrology).
"""
...
LISTING = Path("/tmp/ginger-nephrology.html")
OUT_DOCTORS = Path("/tmp/nephrology-doctors.json")
...
NEPHROLOGY_RULES: list[tuple[re.Pattern, str]] = [
    (re.compile(r"paired kidney|swap transplant|kidney exchange|kidney paired", re.I), "Paired Kidney Exchange (Swap Transplant)"),
    (re.compile(r"abo[\s-]*incompat", re.I), "ABO-Incompatible Kidney Transplantation"),
    (re.compile(r"living donor kidney|live donor kidney", re.I), "Living Donor Kidney Transplantation"),
    (re.compile(r"deceased donor kidney|cadaver.{0,12}kidney", re.I), "Deceased Donor Kidney Transplantation"),
    (re.compile(r"graft biopsy|allograft biopsy|transplant biopsy", re.I), "Kidney Transplant Graft Biopsy"),
    (re.compile(r"transplant evaluation|transplant follow|post[\s-]*transplant", re.I), "Kidney Transplant Evaluation and Follow-up"),
    (re.compile(r"kidney transplant|renal transplant", re.I), "Kidney Transplantation"),
    (re.compile(r"\bcrrt\b|continuous renal replacement", re.I), "Continuous Renal Replacement Therapy (CRRT)"),
    (re.compile(r"\bsled\b|sustained low[\s-]*efficiency", re.I), "Sustained Low-Efficiency Dialysis (SLED)"),
    (re.compile(r"peritoneal dialysis|\bcapd\b|\bapd\b", re.I), "Peritoneal Dialysis"),
    (re.compile(r"hemodialysis|haemodialysis|maintenance dialysis|\bdialysis\b|ckd|esrd|end[\s-]*stage kidney|chronic kidney", re.I), "Hemodialysis"),
    (re.compile(r"capd catheter|pd catheter|peritoneal catheter", re.I), "CAPD Catheter Insertion"),
    (re.compile(r"permcath|perm[\s-]*cath|tunnelled.{0,12}catheter|cuffed catheter", re.I), "Central Venous Catheter (Permcath) Insertion"),
    (re.compile(r"dialysis catheter|vascath|non[\s-]*tunnel", re.I), "Dialysis Catheter Placement"),
    (re.compile(r"av fistula|arteriovenous fistula|cimino", re.I), "AV Fistula Creation"),
    (re.compile(r"dialysis access|fistuloplasty|access thromb", re.I), "Dialysis Access Management"),
    (re.compile(r"renal biopsy|kidney biopsy|native.{0,8}biopsy|glomerulo|nephrotic", re.I), "Percutaneous Renal Biopsy"),
    (re.compile(r"plasmapheresis|plasma exchange|\bplex\b", re.I), "Plasmapheresis"),
]

FALLBACK = ["Hemodialysis", "Kidney Transplantation", "Percutaneous Renal Biopsy"]
...
def map_procedures(texts: list[str]) -> list[str]:
    ...
    for test, name in NEPHROLOGY_RULES:
...
def first_given(name: str) -> str:
    n = name.lower()
    n = re.sub(r"^dr\.?\s*", "", n)
    n = re.sub(r"^\((?:col|prof)\)\s*", "", n)
    n = re.sub(r"^col\.?\s*", "", n)
...
def pronoun_from(text: str, name: str) -> str:
    ...
    if re.search(r"\bhe\b|\bhis\b|\bhim\b", t):
        return "he"
    if first_given(name) in FEMALE_FIRST:
        return "she"
    return "he"
...
def write_bio(d: dict) -> str:
    name = d["name"]
    title = d["designation"] or "Consultant, Nephrology"
    ...
    years_bit = (
        f"{cap} has practised nephrology in India for {years.lower()}."
        if years
        else f"{cap} practises nephrology in India."
    )
    edu_bit = f" Training includes {edu}." if edu else ""
    featured_bit = (
        f" GAF Healthcare lists {obj} among featured India nephrologists for dialysis, transplant and glomerular lists."
        if d.get("featured")
        else ""
    )
    return (
        f"{name} is {title} at {hospital} in {city}, India. {years_bit}{featured_bit} "
        f"Dialysis, transplant, access and biopsy lists are tagged for later pSEO under country, city, specialty and procedure — "
        f"{p1}, {p2} and {p3} when GFR, HLA or histology already make the indication honest. "
        f"HD versus peritoneal dialysis, catheter versus fistula, and medical listing versus the shared Urology transplant slug "
        f"are written after records review — not from a brochure robot or session count.{edu_bit} "
        f"International patients meet {obj} on camera first; travel to {city} is offered only if this nephrology floor is the right list."
    )
