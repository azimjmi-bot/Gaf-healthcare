#!/usr/bin/env python3
"""Fetch structured fields from Ginger's India cosmetic surgery listing. Bios are written here, not copied."""
...
LISTING = Path("/tmp/ginger-cosmetic.html")
...
COSMETIC_RULES: list[tuple[re.Pattern, str]] = [
    ...
    (re.compile(r"gynecomastia|male breast", re.I), "Gynecomastia Surgery"),
    (re.compile(r"breast[\s-]*aug|augmentation|implants", re.I), "Breast Augmentation"),
    (re.compile(r"breast[\s-]*red|reduction mammo", re.I), "Breast Reduction"),
    (re.compile(r"breast[\s-]*lift|mastopexy", re.I), "Breast Lift"),
    (re.compile(r"hair transplant|fue|fut|follicular", re.I), "Hair Transplant"),
    (re.compile(r"tummy tuck|abdominoplasty", re.I), "Tummy Tuck"),
    (re.compile(r"blepharoplasty|eyelid", re.I), "Blepharoplasty"),
    (re.compile(r"otoplasty|ear pinning|prominent ear", re.I), "Otoplasty"),
    (re.compile(r"neck lift|platysma", re.I), "Neck Lift"),
    (re.compile(r"arm lift|brachioplasty", re.I), "Arm Lift"),
    (re.compile(r"facelift|rhytidectomy", re.I), "Facelift"),
    (re.compile(r"rhinoplasty|nose job|septorhino", re.I), "Rhinoplasty"),
    (re.compile(r"fat transfer|fat graft|lipofilling", re.I), "Fat Transfer"),
    (re.compile(r"liposuction|\blipo\b", re.I), "Liposuction"),
]

FEMALE_HINTS = ("suma", "bhumika", "soumya", "mansi", "chandhana", "shraddha", "shweta")

NEW_HOSPITAL_NAMES = {
    "indian-spinal-injuries-centre": "Indian Spinal Injuries Centre",
}

SOURCE = "https://ginger.healthcare/destinations/india/cosmetic-surgery/"
...
def map_procedures(texts: list[str]) -> list[str]:
    ...
    for test, name in COSMETIC_RULES:
        if test.search(blob) and name not in found:
            found.append(name)
    return found or ["Rhinoplasty", "Liposuction", "Breast Augmentation"]
...
def write_bio(d: dict) -> str:
    name = d["name"]
    title = d["designation"] or "Consultant, Cosmetic Surgery"
    ...
    p1 = procs[0] if procs else "Rhinoplasty"
    p2 = procs[1] if len(procs) > 1 else "Liposuction"
    p3 = procs[2] if len(procs) > 2 else "Breast Augmentation"
    edu = d["education"][0] if d["education"] else ""
    pron = d["_pronoun"]
    obj = "her" if pron == "she" else "him"
    cap = "She" if pron == "she" else "He"
    years_bit = (
        f"{cap} has practised plastic and cosmetic surgery in India for {years.lower()}."
        if years
        else f"{cap} practises cosmetic surgery in India."
    )
    edu_bit = f" Training includes {edu}." if edu else ""
    return (
        f"{name} is {title} at {hospital} in {city}, India. {years_bit} "
        f"The cosmetic list covers {p1}, {p2} and {p3} when photographs and anatomy make the indication honest. "
        f"Implant, graft and scar plan are written after records review — not from a brochure.{edu_bit} "
...
def parse_hospital(html: str, slug: str, href: str, city: str, name_hint: str) -> dict:
    ...
    bio = (
        ...
        f"Named plastic surgeons write rhinoplasty, liposuction and breast work here; "
        f"oncology, haematology, cardiac and bariatric lists sit on the same map when the case needs them. "
...
def main() -> None:
    ...
    for d in sorted(enriched, key=lambda x: x["name"]):
        ...
        if not d.get("specializations"):
            d["specializations"] = ["Cosmetic Surgery"]
        doctors.append(d)
